...
{{-- 
@foreach($all1_categorias as $all1_category)
@if($activar == $all1_category->id)
@php $activar = 'Padre: '.$all1_category->id_padre @endphp<br>
@php $all1_active = $all1_category->id @endphp
Padre: {{ $all1_category->id }}

    @foreach($all2_categorias as $all2_category)
    @if($all1_category->id_padre == $all2_category->id)
    @php $activar = 'Hijo: '.$all2_category->id_padre @endphp<br>
    @php $all2_active = $all2_category->id @endphp 
    Hijo: {{ $all2_category->id }}


        @foreach($all3_categorias as $all3_category)
        @if($all2_category->id_padre == $all3_category->id)
        @php $activar = 'Otro: '.$all3_category->id_padre @endphp<br>
        @php $all3_active = $all3_category->id @endphp 
        Otro: {{ $all3_category->id }}
...
Otro: {{$all3_active}}
 --}}
   ...
   <a href="{{route('categoria',$all1_categoria)}}"      class="collapsible-header waves-effect waves-admin fc2 fs18 fw6" style="text-transform: uppercase;">{{$all1_categoria->titulo}}{{$all1_categoria->id}}</a>
   <div class="collapsible-body">
      ...
      <a        class="collapsible-header waves-effect waves-admin fc2 fs18 fw6" style="color:#565656;font-weight: 600; font-size: 16px; letter-spacing: 0.3px">&nbsp;&nbsp;{{$all2_categoria->titulo}}{{$all2_categoria->id}}</a>
      <div class="collapsible-body">
         ...
         <li class="">
         <a         class="collapsible-header waves-effect waves-admin fc2 fs18 fw6" style="color: #a9a9a9; font-size: 16px; letter-spacing: 0.3px">&nbsp;&nbsp;&nbsp;&nbsp;{{$all3_categoria->titulo}}{{$all3_categoria->id}}</a>
         <div class="collapsible-body">
            ...
            <li class="">
            <a         class="collapsible-header waves-effect waves-admin fc2 fs18 fw6" style="color: #9a9999; font-size: 15px; letter-spacing: 0.4px; font-weight: 400;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{{$all4_categoria->titulo}}{{$all4_categoria->id}}</a>
            <div class="collapsible-body">
               ...
               <li class="">
               <a          class="collapsible-header waves-effect waves-admin fc2 fs18 fw6">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{{$all5_categoria->titulo}}{{$all5_categoria->id}}</a>
               <div class="collapsible-body">
...
<div class="col m12 l4">
  ...
  <div class="card z-depth-0">
    ...
    <a href="{{route('categoria', $categoria->id)}}">
        ...
        <img src="{{asset('imagenes/img_cat/'.$categoria->imagen)}}" style="border: 1px solid #DDD; border-top: 5px solid var(--color2);">
...
<div class="col m12 l4">
                <div class="card z-depth-0">
                  <div class="card-image center-align">
                    <a href="{{route('producto', $producto->id)}}">
                      <div class="efecto">
                          <span class="central"><i class="material-icons">add</i>
                          </span>
                      </div>
                    @foreach($galerias as $galeria)
              {{-- 
                        @if($imagenes == '0')
                        <img src="{{asset('imagenes/noimg.jpg')}}" style="border: 1px solid #DDD; width: 100%; height: 100%; border-top: 5px solid var(--color2);">
                        @break
                        @endif
               --}}
                        ...
                        @if($galeria->id_producto == $producto->id)
                                    @if($active == 'ofertas')
                                    <img src="{{asset('imagenes/help/'.$producto->oferta.'.png')}}" style="width: 153px; height: 111px ;position: absolute; z-index: 999;">
                                    @endif          
                        <img src="{{asset('imagenes/galerias/'.$galeria->imagen)}}" style="border: 1px solid #DDD; border-top: 5px solid var(--color2);">
                        @php $imagenes == '1' @endphp
                        @break
                        @endif
    ...
    <div class="card-content cero center-align fw6" style="background: var(--color2); height: 80px; padding: 6px 12px; display: flex; justify-content: center; align-items: center;">
      ...
      <div class="fc6 fw6 fs18">{!!($producto->titulo) !!}</div>
                  </div>
                </div>
              </div>
